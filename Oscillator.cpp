...
Oscillator::Oscillator() : AudioNode() {
	...
	for (int i=0; i<=256; i++) {
		_expFrequency[i] = int64_t(pow(2,((float(i) - 117.0 - 69.0) / 12.0)) * 440.0) << 32;  // divide by sample rate?
	}
...
void Oscillator::setFrequencyInt32() {	// takes a float between -1.0 and 1.0
//	_frequency = (int64_t(_freq) << 32) / SAMPLE_RATE;
